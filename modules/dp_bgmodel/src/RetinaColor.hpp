...
public:
	 ...
	 */
	RetinaColor(const unsigned int NBrows, const unsigned int NBcolumns, const int samplingMethod =
			RETINA_COLOR_BAYER);
	 ...
	 */
	inline void runColorMultiplexing(const std::valarray<float> &inputRGBFrame)
	{
	 ...
	 */
	void runColorMultiplexing(const std::valarray<float> &demultiplexedInputFrame,
	 ...
	 */
	void runColorDemultiplexing(const std::valarray<float> &multiplexedColorFrame,
			const bool adaptiveFiltering = false, const float maxInputValue = 255.0);
	 ...
	 * */
	void setColorSaturation(const bool saturateColors = true,
	 ...
	 */
	inline const std::valarray<float> &getMultiplexedFrame() const
	{
	 ...
	 */
	inline const std::valarray<float> &getDemultiplexedColorFrame() const
	{
	 ...
	 */
	inline const std::valarray<float> &getLuminance() const
	{
		return *_luminance;
	}
	 ...
	 */
	inline const std::valarray<float> &getChrominance() const
	{
		return _chrominance;
	}
	 ...
	 */
	void clipRGBOutput_0_maxInputValue(float *inputOutputBuffer,
	 ...
	 */
	inline void setDemultiplexedColorFrame(const std::valarray<float> &demultiplexedImage)
	{
...
protected:
	...
	void _adaptiveHorizontalCausalFilter_addInput(const float *inputFrame, float *outputFrame,
			const unsigned int IDrowStart, const unsigned int IDrowEnd); // TBB parallelized
	void _adaptiveVerticalAnticausalFilter_multGain(float *outputFrame,
			const unsigned int IDcolumnStart, const unsigned int IDcolumnEnd);
	void _computeGradient(const float *luminance);
	void _normalizeOutputs_0_maxOutputValue(void);

	// color space transform
	void _applyImageColorSpaceConversion(const std::valarray<float> &inputFrame,
			std::valarray<float> &outputFrame, const float *transformTable);
	...
	public:
		Parallel_adaptiveHorizontalCausalFilter_addInput(const float *inputImg,
				float *bufferToProcess, const float *imageGrad, const unsigned int nbCols) :
				outputFrame(bufferToProcess), inputFrame(inputImg), imageGradient(imageGrad), nbColumns(
						nbCols)
		{
		}
	...
	public:
		Parallel_adaptiveVerticalAnticausalFilter_multGain(float *bufferToProcess,
				const float *imageGrad, const unsigned int nbRws, const unsigned int nbCols,
				const float gain) :
				outputFrame(bufferToProcess), imageGradient(imageGrad), nbRows(nbRws), nbColumns(
						nbCols), filterParam_gain(gain)
		{
		}
	...
	public:
		Parallel_computeGradient(const unsigned int nbCols, const unsigned int nbRws,
				const float *lum, float *imageGrad) :
				imageGradient(imageGrad), luminance(lum), nbColumns(nbCols), doubleNbColumns(
						2 * nbCols), nbRows(nbRws), nbPixels(nbRws * nbCols)
		{
		}
