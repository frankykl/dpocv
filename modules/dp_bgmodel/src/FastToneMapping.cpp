...
public:
	...
	RetinaFastToneMappingImpl(Size imageInput)
	{
		...
		if (nbPixels <= 0)
			throw cv::Exception(-1,
					...
					"RetinaImpl::setup", "retinafasttonemapping.cpp", 0);
			...
			CV_OVERRIDE
	{
		...
		if (colorMode) {
			...
			_convertValarrayBuffer2cvMat(_imageOutput, _multiuseFilter->getNBrows(),
					_multiuseFilter->getNBcolumns(), true, outputToneMappedImage);
		} else {
			_runGrayToneMapping(_inputBuffer, _imageOutput);
			_convertValarrayBuffer2cvMat(_imageOutput, _multiuseFilter->getNBrows(),
					_multiuseFilter->getNBcolumns(), false, outputToneMappedImage);
		}
	 ...
	 */
	virtual void setup(const float photoreceptorsNeighborhoodRadius = 3.f,
			const float ganglioncellsNeighborhoodRadius = 1.f, const float meanLuminanceModulatorK =
					1.f) CV_OVERRIDE
	{
...
private:
	...
	void _convertValarrayBuffer2cvMat(const std::valarray<float> &grayMatrixToConvert,
			...
			std::valarray<float> &outputValarrayMatrix)
	{
		...
		if (imageNumberOfChannels == 4) {
			...
			cv::Mat planes[4] = { cv::Mat(inputMatToConvert.size(), dsttype,
					&outputValarrayMatrix[doubleNBpixels]), cv::Mat(inputMatToConvert.size(),
					dsttype, &outputValarrayMatrix[nbPixels]), cv::Mat(inputMatToConvert.size(),
					dsttype, &outputValarrayMatrix[0]) };
...
// run the initilized retina filter in order to perform gray image tone mapping, after this call all retina outputs are updated
	void _runGrayToneMapping(const std::valarray<float> &grayImageInput,
			std::valarray<float> &grayImageOutput)
	{
		...
		_multiuseFilter->setV0CompressionParameterToneMapping(1.f, grayImageOutput.max(),
				_meanLuminanceModulatorK * grayImageOutput.sum()
						/ (float) _multiuseFilter->getNBpixels());
		...
		_multiuseFilter->setV0CompressionParameterToneMapping(1.f, _temp2.max(),
				_meanLuminanceModulatorK * grayImageOutput.sum()
						/ (float) _multiuseFilter->getNBpixels());
...
// run the initilized retina filter in order to perform color tone mapping, after this call all retina outputs are updated
	void _runRGBToneMapping(const std::valarray<float> &RGBimageInput,
			std::valarray<float> &RGBimageOutput, const bool useAdaptiveFiltering)
	{
		...
		_colorEngine->runColorDemultiplexing(RGBimageOutput, useAdaptiveFiltering,
				_multiuseFilter->getMaxInputValue()); //_ColorEngine->getMultiplexedFrame());//_ParvoRetinaFilter->getPhotoreceptorsLPfilteringOutput());
