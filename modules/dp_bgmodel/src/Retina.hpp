...
struct RetinaParameters {
	...
	struct OPLandIplParvoParameters {
		OPLandIplParvoParameters() :
				colorMode(true), normaliseOutput(true), photoreceptorsLocalAdaptationSensitivity(0.75f),
				photoreceptorsTemporalConstant(0.9f), photoreceptorsSpatialConstant(0.53f),
				horizontalCellsGain(0.01f), hcellsTemporalConstant(0.5f), hcellsSpatialConstant(7.f),
				ganglionCellsSensitivity(0.75f)
		{
		} // default setup
		bool colorMode, normaliseOutput;
		float photoreceptorsLocalAdaptationSensitivity, photoreceptorsTemporalConstant,
	...
	struct IplMagnoParameters {
		IplMagnoParameters() :
				normaliseOutput(true), parasolCells_beta(0.f), parasolCells_tau(0.f), parasolCells_k(7.f),
				amacrinCellsTemporalCutFrequency(2.0f), V0CompressionParameter(0.95f),
				localAdaptintegration_tau(0.f), localAdaptintegration_k(7.f)
		{
		} // default setup
		bool normaliseOutput;
		float parasolCells_beta, parasolCells_tau, parasolCells_k, amacrinCellsTemporalCutFrequency,
...
public:
	 ...
	 */
	CV_WRAP
	virtual void setupOPLandIPLParvoChannel(const bool colorMode = true,
	 ...
	 */
	CV_WRAP
	virtual void setupIPLMagnoChannel(const bool normaliseOutput = true,
			const float parasolCells_beta = 0.f, const float parasolCells_tau = 0.f,
	 ...
	 */
	CV_WRAP
	virtual void setColorSaturation(const bool saturateColors = true,
	...
	/** @brief Constructors from standardized interfaces : retreive a smart pointer to a Retina instance
	 ...
	 @param samplingStrenght only usefull if param useRetinaLogSampling=true, specifies the strength of
	 the log scale that is applied
	 */
	CV_WRAP
	static Ptr<Retina> create(Size inputSize, const bool colorMode, int colorSamplingMethod =
			RETINA_COLOR_BAYER, const bool useRetinaLogSampling = false,
