...
public:
	 ...
	 */
	RetinaFilter(const unsigned int sizeRows, const unsigned int sizeColumns, const bool colorMode =
			false, const int samplingMethod = RETINA_COLOR_BAYER, const bool useRetinaLogSampling =
			false, const double reductionFactor = 1.0, const double samplingStrenght = 10.0);
	 ...
	 */
	bool runFilter(const std::valarray<float> &imageInput, const bool useAdaptiveFiltering = true,
			const bool processRetinaParvoMagnoMapping = false, const bool useColorMode = false,
			const bool inputIsColorMultiplexed = false);
	 ...
	 */
	void runGrayToneMapping(const std::valarray<float> &grayImageInput,
			std::valarray<float> &grayImageOutput, const float PhotoreceptorsCompression = 0.6,
			const float ganglionCellsCompression = 0.6);
	 ...
	 */
	void runRGBToneMapping(const std::valarray<float> &RGBimageInput,
			std::valarray<float> &imageOutput, const bool useAdaptiveFiltering,
			const float PhotoreceptorsCompression = 0.6,
			const float ganglionCellsCompression = 0.6);
	 ...
	 * @param ganglionCellsCompression: sets the log compression applied at the gnaglion cells output (enhance contrast)
	 */
	void runLMSToneMapping(const std::valarray<float> &LMSimageInput,
			std::valarray<float> &imageOutput, const bool useAdaptiveFiltering,
			const float PhotoreceptorsCompression = 0.6,
			const float ganglionCellsCompression = 0.6);
	...
	void setGlobalParameters(const float OPLspatialResponse1 = 0.7,
			const float OPLtemporalresponse1 = 1, const float OPLassymetryGain = 0,
			const float OPLspatialResponse2 = 5, const float OPLtemporalresponse2 = 1,
			const float LPfilterSpatialResponse = 5, const float LPfilterGain = 0,
			const float LPfilterTemporalresponse = 0,
			const float MovingContoursExtractorCoefficient = 5,
			const bool normalizeParvoOutput_0_maxOutputValue = false,
			const bool normalizeMagnoOutput_0_maxOutputValue = false, const float maxOutputValue =
	 ...
	 */
	inline void setGanglionCellsLocalAdaptationLPfilterParameters(const float spatialResponse,
			const float temporalResponse)
	{
		_ParvoRetinaFilter.setGanglionCellsLocalAdaptationLPfilterParameters(temporalResponse,
				spatialResponse);
		_setInitPeriodCount();
	}
	 ...
	 */
	void setOPLandParvoParameters(const float beta1, const float tau1, const float k1,
			const float beta2, const float tau2, const float k2, const float V0CompressionParameter)
	{
	...
	void setMagnoCoefficientsTable(const float parasolCells_beta, const float parasolCells_tau,
			...
			const float V0CompressionParameter, const float localAdaptintegration_tau,
			const float localAdaptintegration_k)
	{
		_MagnoRetinaFilter.setCoefficientsTable(parasolCells_beta, parasolCells_tau, parasolCells_k,
				amacrinCellsTemporalCutFrequency, localAdaptintegration_tau,
	 ...
	 */
	inline void activateNormalizeParvoOutput_0_maxOutputValue(
	...
	inline void activateNormalizeMagnoOutput_0_maxOutputValue(
			const bool normalizeMagnoOutput_0_maxOutputValue)
	{
	 ...
	 * */
	inline void setColorSaturation(const bool saturateColors = true,
	 ...
	 */
	inline const std::valarray<float> &getLocalAdaptation() const
	{
	 ...
	 */
	inline const std::valarray<float> &getPhotoreceptors() const
	{
	 ...
	 */
	inline const std::valarray<float> &getHorizontalCells() const
	{
	 ...
	 */
	inline const std::valarray<float> &getMovingContours() const
	{
	 ...
	 */
	inline const std::valarray<float> &getMovingContoursSaturated() const
	{
		return _MagnoRetinaFilter.getMagnoYsaturated();
	} // Saturated Magnocellular output
	 ...
	 */
	inline const std::valarray<float> &getMovingContoursON() const
	{
		return _MagnoRetinaFilter.getMagnoON();
	} // Magnocellular ON output
	 ...
	 */
	inline const std::valarray<float> &getMovingContoursOFF() const
	{
		return _MagnoRetinaFilter.getMagnoOFF();
	} // Magnocellular OFF output
	 ...
	 */
	inline const std::valarray<float> &getRetinaParvoMagnoMappedOutput() const
	{
	 ...
	 */
	inline const std::valarray<float> &getParvoContoursChannel() const
	{
	 ...
	 */
	inline bool applyKrauskopfLMS2Acr1cr2Transform(std::valarray<float> &result)
	{
		return _colorEngine.applyKrauskopfLMS2Acr1cr2Transform(result);
	}
	 ...
	 * @return true if process ended successfully
	 */
	inline bool applyLMS2LabTransform(std::valarray<float> &result)
	{
		return _colorEngine.applyLMS2LabTransform(result);
	}
	...
	}  // return true if RGB mode, false if gray level mode
	...
	{
		return _photoreceptorsLogSampling->getIrregularLPfilteredInputFrame();
	}
	...
	inline float getRetinaSamplingBackProjection(const float projectedRadiusLength)
	{
		if (_photoreceptorsLogSampling)
			return (float) _photoreceptorsLogSampling->getOriginalRadiusLength(
					projectedRadiusLength);
	...
	{
		if (_photoreceptorsLogSampling)
			return _photoreceptorsLogSampling->getNBrows();
		else
	...
	{
		if (_photoreceptorsLogSampling)
			return _photoreceptorsLogSampling->getNBcolumns();
		else
	...
	{
		if (_photoreceptorsLogSampling)
			return _photoreceptorsLogSampling->getNBpixels();
		else
...
protected:
	...
	void _runGrayToneMapping(const std::valarray<float> &grayImageInput,
			std::valarray<float> &grayImageOutput, const float PhotoreceptorsCompression = 0.6,
			const float ganglionCellsCompression = 0.6);
};
