...
public:
	...
	Size getSize()
	{
		return cv::Size(getNBcolumns(), getNBrows());
	}
...
protected:
	 ...
	 */
	inline const std::valarray<float> &getLocalMotionPicture() const
	{
		return _localMotion;
	}
	 ...
	 */
	inline const std::valarray<float> &getNeighborhoodMotionPicture() const
	{
		return _neighborhoodMotion;
	}
	 ...
	 */
	inline const std::valarray<float> &getMotionContextPicture() const
	{
		return _contextMotionEnergy;
	}

	struct SegmentationParameters _segmentationParameters;
	...
	void _convertValarrayBuffer2cvMat(const std::valarray<bool> &grayMatrixToConvert,
			const unsigned int nbRows, const unsigned int nbColumns, OutputArray outBuffer);
	bool _convertCvMat2ValarrayBuffer(InputArray inputMat,
			std::valarray<float> &outputValarrayMatrix);

	const TransientAreasSegmentationModuleImpl & operator =(
			const TransientAreasSegmentationModuleImpl &);
};

class TransientAreasSegmentationModuleImpl_: public TransientAreasSegmentationModule {
public:
	TransientAreasSegmentationModuleImpl_(const Size size) :
			_segmTool(size)
	{
	}
	inline virtual Size getSize() CV_OVERRIDE
	{
		return _segmTool.getSize();
	}
	inline virtual void write(cv::FileStorage& fs) const CV_OVERRIDE
	{
		_segmTool.write(fs);
	}
	inline virtual void setup(String segmentationParameterFile,
			const bool applyDefaultSetupOnFailure) CV_OVERRIDE
	{
		_segmTool.setup(segmentationParameterFile, applyDefaultSetupOnFailure);
	}
	inline virtual void setup(cv::FileStorage &fs, const bool applyDefaultSetupOnFailure)
			CV_OVERRIDE
	{
		_segmTool.setup(fs, applyDefaultSetupOnFailure);
	}
	inline virtual void setup(SegmentationParameters newParameters) CV_OVERRIDE
	{
		_segmTool.setup(newParameters);
	}
	inline virtual const String printSetup() CV_OVERRIDE
	{
		return _segmTool.printSetup();
	}
	inline virtual struct SegmentationParameters getParameters() CV_OVERRIDE
	{
		return _segmTool.getParameters();
	}
	inline virtual void write(String fs) const CV_OVERRIDE
	{
		_segmTool.write(fs);
	}
	inline virtual void run(InputArray inputToSegment, const int channelIndex) CV_OVERRIDE
	{
		_segmTool.run(inputToSegment, channelIndex);
	}
	inline virtual void getSegmentationPicture(OutputArray transientAreas) CV_OVERRIDE
	{
		return _segmTool.getSegmentationPicture(transientAreas);
	}
	inline virtual void clearAllBuffers() CV_OVERRIDE
	{
		_segmTool.clearAllBuffers();
	}
...
TransientAreasSegmentationModuleImpl::TransientAreasSegmentationModuleImpl(const Size size) :
		...
		_inputToSegment(size.height * size.width), _contextMotionEnergy(size.height * size.width), _segmentedAreas(
		...
		_localMotion(_localBuffer), _neighborhoodMotion(_filterOutput)
{
...
// setup from XML file
void TransientAreasSegmentationModuleImpl::setup(String segmentationParameterFile,
		const bool applyDefaultSetupOnFailure)
{
	try {
		...
		if (applyDefaultSetupOnFailure) {
			...
			SegmentationParameters defaults;
			setup(defaults);
		} else {
...
void TransientAreasSegmentationModuleImpl::setup(cv::FileStorage &fs,
		const bool applyDefaultSetupOnFailure)
{
	try {
		...
		currFn["localEnergy_temporalConstant"]
				>> _segmentationParameters.localEnergy_temporalConstant;
		currFn["localEnergy_spatialConstant"]
				>> _segmentationParameters.localEnergy_spatialConstant;
		currFn["neighborhoodEnergy_temporalConstant"]
				>> _segmentationParameters.neighborhoodEnergy_temporalConstant;
		currFn["neighborhoodEnergy_spatialConstant"]
				>> _segmentationParameters.neighborhoodEnergy_spatialConstant;
		currFn["contextEnergy_temporalConstant"]
				>> _segmentationParameters.contextEnergy_temporalConstant;
		currFn["contextEnergy_spatialConstant"]
				>> _segmentationParameters.contextEnergy_spatialConstant;
		setup(_segmentationParameters);

	} catch (cv::Exception &e) {
		std::cout << "Retina::setup: resetting retina with default parameters" << std::endl;
		if (applyDefaultSetupOnFailure) {
			struct SegmentationParameters defaults;
			setup(defaults);
		}
		std::cout
				<< "SegmentationModule::setup: wrong/unappropriate xml parameter file : error report :`n=>"
				<< e.what() << std::endl;
...
void TransientAreasSegmentationModuleImpl::setup(SegmentationParameters newParameters)
{

	// copy structure contents
	memcpy(&_segmentationParameters, &newParameters, sizeof(SegmentationParameters));
	// apply setup
	...
	BasicRetinaFilter::setLPfilterParameters(0, newParameters.localEnergy_temporalConstant,
			newParameters.localEnergy_spatialConstant);
	// init neighbohood motion energy extraction low pass filter
	BasicRetinaFilter::setLPfilterParameters(0, newParameters.neighborhoodEnergy_temporalConstant,
			newParameters.neighborhoodEnergy_spatialConstant, 1);

	// init large area low pass filter
	BasicRetinaFilter::setLPfilterParameters(0, newParameters.contextEnergy_temporalConstant,
			newParameters.contextEnergy_spatialConstant, 2);
...
const String TransientAreasSegmentationModuleImpl::printSetup()
{
	...
	outmessage << "Current segmentation instance setup :" << "\n\t thresholdON : "
			<< _segmentationParameters.thresholdON << "\n\t thresholdOFF : "
			<< _segmentationParameters.thresholdOFF << "\n\t localEnergy_temporalConstant : "
			<< _segmentationParameters.localEnergy_temporalConstant
...
void TransientAreasSegmentationModuleImpl::write(cv::FileStorage& fs) const
{
	...
	fs << "neighborhoodEnergy_temporalConstant"
			<< _segmentationParameters.neighborhoodEnergy_temporalConstant;
	fs << "neighborhoodEnergy_spatialConstant"
			<< _segmentationParameters.neighborhoodEnergy_spatialConstant;
	fs << "contextEnergy_temporalConstant"
			<< _segmentationParameters.contextEnergy_temporalConstant;
...
void TransientAreasSegmentationModuleImpl::run(InputArray inputToProcess, const int channelIndex)
{
	...
	if ((inputToSegment.rows * inputToSegment.cols) != (int) _inputToSegment.size()) {
		...
		throw cv::Exception(-1, errorMsg.str().c_str(), "SegmentationModule::run",
				"SegmentationModule.cpp", 0);
	}
	if (channelIndex >= inputToSegment.channels()) {
		std::stringstream errorMsg;
		errorMsg << "Cannot access channel index " << channelIndex
				<< " on the input matrix with channels quantity = " << inputToSegment.channels();
		throw cv::Exception(-1, errorMsg.str().c_str(), "SegmentationModule::run",
				"SegmentationModule.cpp", 0);
	}
...
void TransientAreasSegmentationModuleImpl::_run(const std::valarray<float> &inputToSegment,
		const int channelIndex)
{
	...
	_squaringSpatiotemporalLPfilter(
			&const_cast<std::valarray<float>&>(inputToSegment)[channelIndex * getNBpixels()],
			&_localMotion[0]);
	...
	float*localMotionPTR = &_localMotion[0], *neighborhoodMotionPTR = &_neighborhoodMotion[0],
			*contextMotionPTR = &_contextMotionEnergy[0];
	...
	for (unsigned int index = 0; index < _filterOutput.getNBpixels();
			++index, ++segmentationPicturePTR, ++localMotionPTR, ++neighborhoodMotionPTR, contextMotionPTR++) {
		...
		if (generalMotionContextDecision > 0) // local maximum should be detected in this case
			...
			if (generalMotionContextDecision > _segmentationParameters.thresholdON) // && meanEnergy*1.1<*neighborhoodMotionPTR)
					{
				*segmentationPicturePTR = ((*localMotionPTR - *neighborhoodMotionPTR)
						> _segmentationParameters.thresholdON);
			} else
...
void TransientAreasSegmentationModuleImpl::_convertValarrayBuffer2cvMat(
		const std::valarray<bool> &grayMatrixToConvert, const unsigned int nbRows,
		const unsigned int nbColumns, OutputArray outBuffer)
{
...
bool TransientAreasSegmentationModuleImpl::_convertCvMat2ValarrayBuffer(InputArray inputMat,
		std::valarray<float> &outputValarrayMatrix)
{
	const Mat inputMatToConvert = inputMat.getMat();
	// first check input consistency
	if (inputMatToConvert.empty())
		throw cv::Exception(-1, "RetinaImpl cannot be applied, input buffer is empty",
				"RetinaImpl::run", "RetinaImpl.h", 0);
