...
/*
 * Extract foreground color histogram.
 */
void Histogram::ExtractForegroundHistogram(std::vector<cv::Mat> & imgChannels, cv::Mat weights,
	...
	if (!useMatWeights) {
		...
		for (int y = y1; y < y2 + 1; ++y) {
			...
			for (int x = x1; x < x2 + 1; ++x) {
				weightPtr[x] = KernelProfileEpanechnikov(
						std::pow((cx - x) * kernelSize_width, 2) + tmp_y);
			}
		}
...
/*
 * Extract background color histogram.
 */
void Histogram::ExtractBackGroundHistogram(std::vector<cv::Mat> & imgChannels, int x1, int y1,
		int x2, int y2, int outer_x1, int outer_y1, int outer_x2, int outer_y2)
{
	//extract pixel values and compute histogram
	double rangePerBinInverse = static_cast<double>(m_numBinsPerDim) / 256.0; // 1 / (imgRange/numBinsPerDim)
	double sum = 0;
 ...
 * Compute posteriors.
 */
std::pair<cv::Mat, cv::Mat> Segment::ComputePosteriors(std::vector<cv::Mat> &imgChannels, int x1,
		...
		const Histogram &fgHistPrior, int numBinsPerChannel)
{
	...
	Histogram hist_target =
			(fgHistPrior.m_numBinsPerDim == numBinsPerChannel
					&& (size_t) fgHistPrior.m_numDim == imgChannels.size()) ?
					fgHistPrior :
	...
	hist_background.ExtractBackGroundHistogram(imgChannels, x1, y1, x2, y2, outer_x1, outer_y1,
			outer_x2, outer_y2);
	...
	cv::Mat backgroundLikelihood = hist_background.BackProject(imgChannelsROI_inner).mul(
			bgPriorScaled);

	double p_b = std::sqrt(
			(std::pow(outer_x2 - outer_x1, 2) + std::pow(outer_y2 - outer_y1, 2))
	...
	cv::resize(sizedProbs.second, probs.second,
			cv::Size(roiRect_inner.width, roiRect_inner.height));
 ...
 * Compute posteriors.
 */
std::pair<cv::Mat, cv::Mat> Segment::ComputePosteriors2(std::vector<cv::Mat> &imgChannels, int x1,
		int y1, int x2, int y2, double p_b, cv::Mat fgPrior, cv::Mat bgPrior, Histogram hist_target,
...
/*
 * Get regularized segmentation.
 */
std::pair<cv::Mat, cv::Mat> Segment::GetRegularizedSegmentation(cv::Mat &prob_o, cv::Mat &prob_b,
		cv::Mat & prior_o, cv::Mat & prior_b)
{
	int hsize = cvFloor(
			std::max(1.0, (double) cvFloor(static_cast<double>(prob_b.cols) * 3. / 50. + 0.5)));
