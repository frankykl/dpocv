...
Mat CircularShift(Mat matrix, int dx, int dy)
{
	...
	for (int i = 0; i < matrix.rows; i++) {
		for (int j = 0; j < matrix.cols; j++) {
			idx_y = Modulo(i + dy + 1, matrix.rows);
			idx_x = Modulo(j + dx + 1, matrix.cols);
...
/*
 * Generate Gaussian shaped labels.
 */
Mat GaussianShapedLabels(const float sigma, const int w, const int h)
{
	...
	for (int i = 0; i < y.rows; i++) {
		for (int j = 0; j < y.cols; j++) {
			y.at<float>(i, j) = (float) exp(
					(-0.5 / pow(sigma, 2)) * (pow((i + 1 - h2), 2) + pow((j + 1 - w2), 2)));
		}
	}
	// wrap-arround with the circulat shifting
	y = CircularShift(y, -cvFloor(y.cols / 2), -cvFloor(y.rows / 2));
	Mat yf;
...
/*
 * Compute Fourier transform on features.
 */
...
float SubpixelPeak(const Mat &response, const std::string &s, const Point2f &p)
{
	...
	if (s.compare("vertical") == 0) {
		// neighbouring rows
		i_p0 = cvRound(p.y);
		i_p_l = Modulo(cvRound(p.y) - 1, response.rows);
		i_p_r = Modulo(cvRound(p.y) + 1, response.rows);
		...
		// neighbouring cols
		i_p0 = cvRound(p.x);
		i_p_l = Modulo(cvRound(p.x) - 1, response.cols);
		i_p_r = Modulo(cvRound(p.x) + 1, response.cols);
 ...
 * Get HOG features.
 */
std::vector<Mat> GetFeaturesHOG(const Mat &im, const int binSize)
{
	//printf("GetFeaturesHOG\n");
	Mat hogmatrix;
	Mat im_;
	...
	// comput HOG feature
	computeHOG32D(im_, hogmatrix, binSize, 1, 1);
	// convert HOG feature to float
	hogmatrix.convertTo(hogmatrix, CV_32F);

	// reshape HOG feature
	Size hogSize = im.size();
	hogSize.width /= binSize;
	hogSize.height /= binSize;
	Mat hogc(hogSize, CV_32FC(32), hogmatrix.data);
...
/*
 * Get ColorName features.
 */
std::vector<Mat> GetFeaturesCN(const Mat &ppatch_data, const Size &output_size)
{
	...
	for (int i = 0; i < patch_data.rows; i++) {
		for (int j = 0; j < patch_data.cols; j++) {
			...
			index = (unsigned) (cvFloor((float) pixel[2] / 8) + 32 * cvFloor((float) pixel[1] / 8)
					+ 32 * 32 * cvFloor((float) pixel[0] / 8));

			// copy the values
			for (int k = 0; k < 10; k++) {
				cnFeatures.at<Vec<float, 10> >(i, j)[k] = (float) DPColorNames[index][k];
			}
		}
	}
