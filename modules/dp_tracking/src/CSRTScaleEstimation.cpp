...
/*
 * Parallel implementation class.
 */
class ParallelGetScaleFeatures: public ParallelLoopBody {
public:
	ParallelGetScaleFeatures(Mat img, Point2f pos, Size2f base_target_sz, float current_scale,
			std::vector<float> &scale_factors, Mat scale_window, Size scale_model_sz, int col_len,
			Mat &result)
	{
		this->img = img;
		this->pos = pos;
	...
	virtual void operator ()(const Range& range) const CV_OVERRIDE
	{
		for (int s = range.start; s < range.end; s++) {
			...
			Mat img_patch = GetSubwindow(img, pos, patch_sz.width, patch_sz.height);
			...
			std::vector<Mat> hog;
			hog = GetFeaturesHOG(img_patch, 4);
 ...
 * Constructor.
 */
DSST::DSST(const Mat &image, Rect2f bounding_box, Size2f template_size, int numberOfScales,
		float scaleStep, float maxModelArea, float sigmaFactor, float scaleLearnRate) :
		scales_count(numberOfScales), scale_step(scaleStep), max_model_area(maxModelArea), sigma_factor(
				sigmaFactor), learn_rate(scaleLearnRate)
{
	...
	min_scale_factor = pow(scale_step, cvCeil(log(max(5.0 / template_size.width,
			5.0 / template_size.height)) / log(scale_step)));
 ...
 * Get scale features.
 */
Mat DSST::GetScaleFeatures(Mat img, Point2f pos, Size2f base_target_sz, float current_scale,
		std::vector<float> &scale_factors, Mat scale_window, Size scale_model_sz)
{
	...
	std::vector<Mat> hog;
	hog = GetFeaturesHOG(img_patch, 4);
	result = Mat(Size((int) scale_factors.size(), hog[0].cols * hog[0].rows * (int) hog.size()),
			CV_32F);
	...
	ParallelGetScaleFeatures parallelGetScaleFeatures(img, pos, base_target_sz, current_scale,
			scale_factors, scale_window, scale_model_sz, col_len, result);
	parallel_for_(Range(1, static_cast<int>(scale_factors.size())), parallelGetScaleFeatures);
	return result;
}

/*
 * Update with new object center.
 */
void DSST::Update(const Mat &image, const Point2f object_center)
{
	Mat scale_features = GetScaleFeatures(image, object_center, original_targ_sz,
			current_scale_factor, scale_factors, scale_window, scale_model_sz);
	Mat Fscale_features;
...
{
	// get scale features
	Mat scaleFeatures = GetScaleFeatures(image, object_center, original_targ_sz,
			current_scale_factor, scale_factors, scale_window, scale_model_sz);

	// compute FFT
	Mat scaleFeaturesF;
	dft(scaleFeatures, scaleFeaturesF, DFT_ROWS | DFT_COMPLEX_OUTPUT);

	// find best scale in frequency domain
	mulSpectrums(scaleFeaturesF, sf_num, scaleFeaturesF, 0, false);
	Mat scaleResp;
	reduce(scaleFeaturesF, scaleResp, 0, CV_REDUCE_SUM, -1);
	scaleResp = DivideComplexMatrices(scaleResp, sf_den + 0.01f);
	idft(scaleResp, scaleResp, DFT_REAL_OUTPUT | DFT_SCALE);
	Point max_loc;
	minMaxLoc(scaleResp, NULL, NULL, NULL, &max_loc);

	// clip scale factor range
